...
int adios_declare(int64_t *gh, const char *transport_method, const char* opts, char *transform_type_str)
{
	int i;
	int64_t var_id[12]; 
	...
	for (i=0;i<12;i++) 
		adios_set_transform(var_id[i],transform_type_str);
...
int main(int argc, char *argv[])
{
	...
	char *transform_type = argv[6];

	/* Create adios structure */
	int64_t gh;
	adios_init_noxml(comm);
	adios_declare(&gh,transport_method,transport_opts,transform_type);
	...
	for (i=comm_rank;i<num_states;i+=comm_size) {
		...
		fclose(fpp);
		output_step++;
		if (output_step%NUM_STEPS_PER_FILE==0) {
			sprintf(bp_file_now,"%s%d",bp_file,output_step);
			first_time = 1;
			sprintf(fmode,"w");
			if (comm_rank==0) {
