...
class Exaalt(Campaign):
	...
	supported_machines = ['local_fob']

	sweeps = [
		p.SweepGroup (nodes=4,
			post_processing = "",
			...
			[
				p.Sweep([
					...
					p.ParamRunner("exaalt", "nprocs", [1,2,4,8,16]),
					p.ParamCmdLineArg("exaalt", "states_list_file", 1, ["states_list.txt"]),
					p.ParamCmdLineArg("exaalt", "no_of_states", 2, [8,10]),
					p.ParamCmdLineArg("exaalt", "no_of_randoms", 3, [8,10]),
					...
					p.ParamRunner("stage_write", "nprocs", [1,2,4]),
					...
					#p.ParamCmdLineArg("stage_write", "decomposition", 9, []),
				]),
			]),
	]